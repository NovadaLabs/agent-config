 ...
 */
function generateFrontmatter(date, project, tags, summary) {
 ...
 */
function updateJsonlIndex(project) {
    const dir = journalDir(project);
    ensureDir(dir);
 ...
 */
function autoCompactIfNeeded(project) {
    const entries = listJournalFiles(project);
...
server.registerTool("journal_write", {
    ...
    description: "Append content to the current journal entry (creates today's file if absent). Optionally also write key content to this palace room for cross-referenced persistent memory.",
    inputSchema: {
